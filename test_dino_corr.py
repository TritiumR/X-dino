...
def compute_patch_cosine_similarity(features, features_2, pixel_position, original_size):
    ...
    patch_features = F.interpolate(patch_features, size=(h // SCALE_FACTOR, w // SCALE_FACTOR), mode='bilinear', align_corners=False).squeeze(0)  # (C, h, w)
    patch_features = patch_features.permute(1, 2, 0)  # (h, w, C)

    pixel_features = patch_features[pixel_position[0] // SCALE_FACTOR, pixel_position[1] // SCALE_FACTOR, :]
    ...
    patch_features_2 = F.interpolate(patch_features_2, size=(h // SCALE_FACTOR, w // SCALE_FACTOR), mode='bilinear', align_corners=False).squeeze(0)  # (C, h, w)
    ...
    similarity = similarity.reshape(h // SCALE_FACTOR, w // SCALE_FACTOR)
...
for file_name in os.listdir(args.folder_path):
    ...
    for point_id, point in enumerate(points):
        ...
        u = int(most_similar_patch % (w / SCALE_FACTOR)) * SCALE_FACTOR  # x-coordinate (column)
        v = int(most_similar_patch / (w / SCALE_FACTOR)) * SCALE_FACTOR  # y-coordinate (row)
